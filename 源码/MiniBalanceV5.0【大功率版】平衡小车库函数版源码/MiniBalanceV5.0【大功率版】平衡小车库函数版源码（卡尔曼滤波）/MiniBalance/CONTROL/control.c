      ...
      Voltage_Temp=Get_battery_volt();		                                //=====读取电池电压		
  		...
  		if(Bi_zhang==0)Led_Flash(100);                                      //===LED闪烁;常规模式 1s改变一次指示灯的状态	
			...
			Key();                                                              //===扫描按键状态 单击双击可以改变小车运行状态
 			Balance_Pwm=Mycarbl(Angle_Balance,Gyro_Balance);                   //===平衡PID控制	
		  Velocity_Pwm=Mycarvl(Encoder_Left,Encoder_Right);                  //===速度环PID控制	 记住，速度反馈是正反馈，就是小车快的时候要慢下来就需要再跑快一点
 	    Turn_Pwm    =Mycarturn(Encoder_Left,Encoder_Right,Gyro_Turn);            //===转向环PID控制     
...
{  
   float error;
	 int Car_balance;
	 error=angle_in-jxzz;       //===求出平衡的角度中值 和机械相关
	 Car_balance=Balance_Kp*error+Gyro_in*Balance_Kd;   //===计算平衡控制的电机PWM  PD控制   kp是P系数 kd是D系数 
	 return Car_balance;
}
...
{  
    static float Car_Velocity,Enc_Least,Enc,Car_Movement,Car_bizhang;
	  ...
	  //=============遥控前进后退部分=======================// 
	  if(Bi_zhang==1&&Flag_sudu==1)  Car_Target_V=45;                 //如果进入避障模式,自动进入低速模式
    else 	                         Car_Target_V=110;                 
		if(1==Flag_Qian)    	Car_Movement=-Car_Target_V/Flag_sudu;	         //===前进标志位置1 
		else if(1==Flag_Hou)	Car_Movement=Car_Target_V/Flag_sudu;         //===后退标志位置1
	  else  Car_Movement=0;	
	  if(Bi_zhang==1&&Distance<500&&Flag_Left!=1&&Flag_Right!=1)        //避障标志位置1且非遥控转弯的时候，进入避障模式
	  Car_Movement=Car_Target_V/Flag_sudu;
   //=============速度PI控制器=======================//	
		Enc_Least =(enc_l+enc_r)-0;                    //===获取最新速度偏差==测量速度（左右编码器之和）-目标速度（此处为零） 
		Enc *= 0.8;		                                                //===一阶低通滤波器       
		Enc += Enc_Least*0.2;	                                    //===一阶低通滤波器    
		Enc_I +=Enc;                                       //===积分出位移 积分时间：10ms
		Enc_I=Enc_I-Car_Movement;                       //===接收遥控器数据，控制前进后退
		if(Enc_I>10000)  	Enc_I=10000;             //===积分限幅
		if(Enc_I<-10000)	Enc_I=-10000;              //===积分限幅	
		Car_Velocity=Enc*Velocity_Kp+Enc_I*Velocity_Ki;                          //===速度控制	
		if(Turn_Off(Angle_Balance,Voltage)==1||Flag_Stop==1)   Enc_I=0;      //===电机关闭后清除积分
	  return Car_Velocity;
}
...
作    者：平衡小车之家
**************************************************************************/
int Mycarturn(int enc_l,int enc_r,float gyro_z_in)//转向控制
{
...
//		}			
    ...
    if(Car_Tar_Turn>Car_turn_Amp)  Car_Tar_Turn=Car_turn_Amp;    //===转向速度限幅
	  if(Car_Tar_Turn<-Car_turn_Amp) Car_Tar_Turn=-Car_turn_Amp;
		if(Flag_Qian==1||Flag_Hou==1)  Kd=0.5;        
		else Kd=0;   //转向的时候取消陀螺仪的纠正 有点模糊PID的思想
  	//=============转向PD控制器=======================//
		Car_turn=-Car_Tar_Turn*Kp -gyro_z_in*Kd;                 //===结合Z轴陀螺仪进行PD控制
	  return Car_turn;
}
...
{ 		   
	...
	if(flag==0)                                                                   //第一步
	 ...
	 if(flag==1)                                                                  //进入第二步
	 {
	      ...
	      if(Acceleration>26000&&(Angle>(-20+jxzz))&&(Angle<(20+jxzz)))   //条件2，小车是在0度附近被拿起
		    flag=2; 
	 } 
	 if(flag==2)                                                                  //第三步
	 {
...
{ 		   
   ...
   return 0;	                 
	 if(flag==0)                                               
	 {
	      if(Angle>(-10+jxzz)&&Angle<(10+jxzz)&&encoder_left==0&&encoder_right==0)         //条件1，小车是在0度附近的
		    flag=1; 
	 } 
	 if(flag==1)                                               
	 {
